...
import { removeWishlistItemAsync } from "@/redux/features/wishlist-slice";
...
const SingleItem: React.FC<SingleItemProps> = ({ item }) => {
  ...
  const isRemovingFromWishlist = removeStatus === 'pending';
    ...
    <div className="flex items-center border-t border-gray-3 py-5 px-10">
      <div className="min-w-[83px]">
        <button
          ...
          className={`flex items-center justify-center rounded-lg max-w-[38px] w-full h-9.5 border ease-out duration-200 ${isRemovingFromWishlist
              ? 'bg-gray-400 border-gray-400 text-gray-600 cursor-not-allowed'
              : 'bg-gray-2 border-gray-3 hover:bg-red-light-6 hover:border-red-light-4 hover:text-red'
            }`}
        >
      ...
      <div className="min-w-[205px]">
        <p className="text-dark">LKR {item.discountedPrice?.toFixed(2) || '0.00'}</p>
      </div>
      ...
      <div className="min-w-[150px] flex justify-end">
        <button
          ...
          className={`inline-flex items-center gap-2 py-2.5 px-6 rounded-md ease-out duration-200 ${isAddingToCart
