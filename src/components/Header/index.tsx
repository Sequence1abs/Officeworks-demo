...
import { fetchCategoriesWithCounts, selectCategories } from "@/redux/features/category-slice";
...
const Header = () => {
  ...
  const updateStatus = useAppSelector(
    (state) => state.cartReducer.updateStatus
  );
  ...
  const wishlistStatus = useAppSelector(
    (state) => state.wishlistReducer.status
  );
  const wishlistAddStatus = useAppSelector(
    (state) => state.wishlistReducer.addStatus
  );
  const wishlistRemoveStatus = useAppSelector(
    (state) => state.wishlistReducer.removeStatus
  );
  const wishlistClearStatus = useAppSelector(
    (state) => state.wishlistReducer.clearStatus
  );
  const wishlistCheckStatus = useAppSelector(
    (state) => state.wishlistReducer.checkStatus
  );
  ...
  // Check if any wishlist operation is loading
  const isWishlistLoading =
    wishlistStatus === "pending" ||
    wishlistAddStatus === "pending" ||
    wishlistRemoveStatus === "pending" ||
    wishlistClearStatus === "pending" ||
    wishlistCheckStatus === "pending";
  const hasWishlistError =
    wishlistStatus === "failed" ||
    wishlistAddStatus === "failed" ||
    wishlistRemoveStatus === "failed" ||
    wishlistClearStatus === "failed" ||
    wishlistCheckStatus === "failed";
  ...
  // Sticky menu: shadow + compact padding when scrolled past 80px
  const handleStickyMenu = () => {
    setStickyMenu(window.scrollY >= 80);
  };

  useEffect(() => {
    handleStickyMenu(); // sync on mount (e.g. page loaded while scrolled)
  ...
  useEffect(() => {
    if (!categories || categories.length === 0) {
      dispatch(fetchCategoriesWithCounts() as any);
    }
  }, [categories, dispatch]);
  ...
  return (
    <>
      <header
        className={`fixed left-0 top-0 w-full z-10 bg-white transition-all ease-in-out duration-300 ${stickyMenu ? "shadow-md" : "shadow-none"
          }`}
      >
        <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
          {/* <!-- header top start --> */}
          <div
            className={`flex flex-col lg:flex-row gap-5 items-end lg:items-center xl:justify-between transition-[padding] ease-in-out duration-300 ${stickyMenu ? "py-4" : "py-6"
              }`}
          >
            {/* <!-- header top left --> */}
              ...
              <div className="max-w-[475px] w-full">
                <div className="flex items-center">
                  ...
                  <button
                    ...
                    className="relative max-w-[333px] sm:min-w-[333px] w-full text-left rounded-r-[5px] bg-gray-1 !border-l-0 border border-gray-3 py-2.5 pl-4 pr-10 outline-none ease-in duration-200 hover:ring-2 hover:ring-blue/20"
                  >
                    <span className="absolute left-0 top-1/2 -translate-y-1/2 inline-block w-px h-5.5 bg-gray-4"></span>
                    <span className="text-dark-4">I am shopping for...</span>
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-dark-3">
                      <svg
        ...
        <div className="border-t border-gray-3">
          <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
            <div className="flex items-center justify-between">
              {/* <!--=== Main Nav Start ===--> */}
              <div
                className={`w-[288px] absolute right-4 top-full z-50 xl:static xl:w-auto xl:z-auto h-0 xl:h-auto invisible xl:visible xl:flex items-center justify-between ${navigationOpen &&
                  `!visible bg-white shadow-lg border border-gray-3 !h-auto max-h-[85vh] overflow-y-auto rounded-md p-5`
                  }`}
              >
                <nav>
              ...
              <div className="hidden xl:block">
                <ul className="flex items-center gap-5.5">
                  ...
                  <li className="py-4">
                    <Link
                      href="/wishlist"
                      className={`flex items-center gap-1.5 font-medium text-custom-sm ${isWishlistLoading
                        ? "text-blue"
                    ...
                    >
                      <span className="relative">
                        <svg
                          className={`fill-current ${isWishlistLoading ? "animate-pulse" : ""
                            }`}
                          width="16"
                          height="16"
                          viewBox="0 0 16 16"
                        ...
                        {isWishlistLoading && (
                          <span className="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center">
                            <span className="w-1.5 h-1.5 bg-blue rounded-full animate-ping opacity-75"></span>
                          </span>
                        )}
                      </span>
                      <div className="flex flex-col">
                        ...
                        {wishlistItems.length > 0 && (
                          <span className="text-2xs text-blue">
